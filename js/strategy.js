...
(function (ST) {
  ...
  function compute(stock, strat) {
    ...
    // 止盈/止损按持仓成本口径：有持仓时相对持仓平均成本，无持仓时回退纳入价累计涨跌
    const h = ST.Holding ? ST.Holding.compute(stock, ST.Storage ? ST.Storage.getOperations() : []) : { qty: 0 };
    const cum = posChangePct(stock, h);
    ...
    factors.push({ key: "止盈", pass: f_takeProfit, text: `持仓涨幅 ${r2pct(cum)}% ${f_takeProfit ? "≥" : "<"} 止盈线 ${strat.takeProfit}%` });
    factors.push({ key: "止损", pass: f_stopLoss, text: `持仓跌幅 ${r2pct(cum)}% ${f_stopLoss ? "≤" : ">"} 止损线 -${strat.stopLoss}%` });
    ...
    if (f_stopLoss) {
      signal = "sell";
      basis = `已触发止损：持仓 ${r2pct(cum)}% 跌破 -${strat.stopLoss}% 风控线，建议卖出避险。`;
    } else if (f_takeProfit) {
      signal = "sell";
      basis = `已触达止盈：持仓 ${r2pct(cum)}% 达到 ${strat.takeProfit}% 目标，可卖出兑现。`;
    } else if (f_belowLong) {
      signal = "sell";
  ...
  function docHTML(strat) {
    ...
    <ul>
      ...
      <li>持仓跌幅未触及 -${strat.stopLoss}% 止损线</li>
    </ul>
    ...
    <p>满足以下任一条件即触发：</p>
    <ul>
      <li>持仓跌幅 ≤ -${strat.stopLoss}%（止损风控）</li>
      <li>持仓涨幅 ≥ ${strat.takeProfit}%（止盈兑现）</li>
  ...
  //   sell_partial:   { gainPct, ratio }            —— 持仓涨幅达标卖出一部分
  ...
  // 持仓口径的累计涨跌：相对当前持仓平均成本（含手续费摊薄）；
  // 无持仓时回退纳入价累计涨跌（此时不会触发卖出，仅作展示口径）
  function posChangePct(stock, h) {
    if (h && h.qty > 0 && h.avgCost > 0) {
      return (stock.currentPrice - h.avgCost) / h.avgCost * 100;
    }
    return stock.cumChangePct || 0;
  }

  function sellPartialCondition(stock, p, pos) {
    return pos >= (p.gainPct != null ? p.gainPct : 25);
  }
  function sellAllCondition(stock, p, pos) {
    if (p.mode === "stoploss") {
      return pos <= -(Math.abs(p.lossPct != null ? p.lossPct : 8));
    }
  ...
  function evaluate(stock, strategies, allOps) {
    ...
    const pos = posChangePct(stock, h);   // 止盈/止损按持仓成本口径计算
    ...
    strategies.forEach(s => {
      ...
      if (s.attr === "buy") {
        ...
        const pass = sellPartialCondition(stock, s.params || {}, pos);
        factors.push({ key: `${ATTR_TEXT[s.attr]}·${s.name}`, pass, text: pass ? "持仓涨幅达标" : "持仓涨幅未达" });
        if (pass && holding && !hits.sell_partial) hits.sell_partial = s;
      } else if (s.attr === "sell_all") {
        const pass = sellAllCondition(stock, s.params || {}, pos);
        factors.push({ key: `${ATTR_TEXT[s.attr]}·${s.name}`, pass, text: pass ? "条件满足" : "条件未满足" });
    ...
    if (hits.sell_all) {
      ...
      if (partialDone) {
        ...
        basis = `卖出策略【${hits.sell_partial.name}】触发：持仓涨幅达标，建议卖出约 ${Math.round(ratio * 100)}% 部分止盈。`;
      }
    } else if (hits.buy) {
